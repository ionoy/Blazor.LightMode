...
import { createCircuitTransport, createLocationChangingHandler, LightModeFetchResponse, LightModeResponse } from '../src/CircuitTransport';

test('settles normal location-changed response chain', async () => {
    const fetchEndpoints: string[] = [];
    const appliedBatches: string[] = [];

    const transport = createCircuitTransport({
        fetch: async (uri) => {
...
test('rejects transport failures without hanging', async () => {
    ...
    await assert.rejects(
        settleWithin(transport.invoke('_locationChanged', { RequestId: 'req-2', Location: '/broken' })),
        /network down/
    );
});
...
test('location-changing callback settles and calls endLocationChanging', async () => {
    ...
    const locationChanging = createLocationChangingHandler((callId, shouldContinue) => {
...
test('navigation stress keeps render-batch processing active', async () => {
    ...
    const pendingCalls = Array.from({ length: 25 }, (_, index) =>
        transport.invoke('_locationChanged', {
...
function createResponse(serializedRenderBatches: string[], renderCompleted: boolean, needsAfterRender: boolean, responseId: number | null = null): LightModeResponse {
    return {
...
function createJsonResponse(payload: LightModeResponse, status = 200): LightModeFetchResponse {
    return {
        status,
